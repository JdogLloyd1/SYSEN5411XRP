...
class Webserver:
    ...
    def __init__(self):
        ...
        self.fields = {}  # Store text fields and their callbacks
    ...
    def _index_page(self, request):
        ...
        if request.method == 'POST':
            # Check if this is a field submission (has both field name and text input)
            form_keys = list(request.form.keys())
            form_values = list(request.form.values())
            
            # Field submissions have format: field_name_submit as button name, and field_name_input as text input
            if len(form_keys) >= 2 and any(key.endswith('_submit') for key in form_keys):
                # This is a field submission
                field_name = None
                text_input = None
                for key in form_keys:
                    if key.endswith('_submit'):
                        field_name = key[:-7]  # Remove '_submit' suffix
                    elif key.endswith('_input'):
                        text_input = request.form[key]
                
                if field_name and text_input is not None:
                    self._handleUserFieldRequest(field_name, text_input)
                else:
                    # Fallback to button handling
                    if str(form_values[0]).count(" ") == 0:
                        text = str(form_keys[0])
                    else:
                        text = str(form_values[0])
                    self._handleUserFunctionRequest(text)
            else:
                # This is a button submission
                if str(form_values[0]).count(" ") == 0:
                    text = str(form_keys[0])
                else:
                    text = str(form_values[0])
    ...
    def add_field(self, field_name:str, function, placeholder:str=""):
        """
        Register a custom text input field to be displayed on the webserver

        :param field_name: The label for the field as it will be displayed, must be unique
        :type field_name: str
        :param function: The function to be called when the field is submitted, receives the text input as an argument
        :type function: function
        :param placeholder: Placeholder text for the input field, defaults to empty string
        :type placeholder: str, optional
        """
        self.fields[field_name] = {"function": function, "placeholder": placeholder}
    ...
    def _handleUserFieldRequest(self, field_name, text_input) -> bool:
        print(f"Running field {field_name} with input: {text_input}")
        try:
            if field_name not in self.fields:
                logging.warning("User field "+field_name+" not found")
                return False
            user_function = self.fields[field_name]["function"]
            if user_function is None:
                logging.warning("User field function "+field_name+" not found")
                return False
            user_function(text_input)
            return True
        except RuntimeError as xcpt:
            logging.error("User field "+field_name+" caused an exception: "+str(xcpt))
            return False
    ...
    def _generateHTML(self):
        ...
        if self.fields:
            string += f'<h3>Custom Text Fields:</h3>'
            for field_name, field_data in self.fields.items():
                placeholder = field_data["placeholder"]
                string += f'<p><form method="post">'
                string += f'{field_name}: <input type="text" class="text-field" name="{field_name}_input" placeholder="{placeholder}" />'
                string += f' <input type="submit" class="user-button" name="{field_name}_submit" value="Submit" />'
                string += f'</form></p>\n'
