...
# Binding functions to custom text fields
webserver.add_field("Set Servo Angle", lambda angle: servo_one.set_angle(int(angle)), "Enter angle (0-180)")
webserver.add_field("Drive Distance", lambda distance: drivetrain.straight(float(distance)), "Distance in inches")
webserver.add_field("Set Motor Speed", lambda speed: drivetrain.set_effort(float(speed), float(speed)), "Speed (-1.0 to 1.0)")
webserver.add_field("Custom Message", lambda msg: print(f"Received message: {msg}"), "Type your message")
